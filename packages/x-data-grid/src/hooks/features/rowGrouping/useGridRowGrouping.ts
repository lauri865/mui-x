    ...
    (params) => {
      ...
      const traverse = (rowId: GridRowId): number => {
        ...
        const shouldApplyFilterToRow =
          node.type === 'group' || node.type === 'footer'
        ...
        if (!isMatching) {
          ...
          if (isMatching && node.type === 'group') {
            result.filteredChildrenCountLookup[rowId] = node.children.length;
          }
        }

        // If it's a group, traverse its children

        if (!isMatching && node.type === 'group' && node.children) {

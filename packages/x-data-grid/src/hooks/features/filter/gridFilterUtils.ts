 ...
 * @param {RefObject<GridPrivateApi>} apiRef The API of the grid.
 * @return {GridFilterItem} The clean filter item with an uniq ID and an always-defined operator.
 * TODO: Make the typing reflect the different between GridFilterInputItem and GridFilterItem.
 */
export const cleanFilterItem = (item: GridFilterItem, apiRef: RefObject<GridPrivateApi>) => {
...
export const sanitizeFilterModel = (
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (hasItemsWithoutIds) {
      warnOnce(
        'TWGrid: The `id` field is required on `filterModel.items` when you use multiple filters.',
        'error',
      );
    }
  }
  ...
  if (process.env.NODE_ENV !== 'production') {
    if (hasItemWithoutOperator) {
      warnOnce(
        'TWGrid: The `operator` field is required on `filterModel.items`, one or more of your filtering item has no `operator` provided.',
        'error',
      );
    }
  }
...
export const mergeStateWithFilterModel =
  ...
  (filteringState: GridState['filter']): GridState['filter'] => ({
    ...filteringState,
